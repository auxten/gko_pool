...
void ev_callback(int fd, short ev, void *arg)
{
    if (ev & EV_READ)
...
////////////////////
    ...
    {
//        printf( "read %d\n", *(read_fds + i));
        event_set(&ev_dns, *(read_fds + i), EV_READ | EV_PERSIST, ev_callback,
                (void *) (channel));
    }
    for (int i = 0; *(write_fds + i) != ARES_SOCKET_BAD; i++)
    {
//        printf("write %d\n", *(write_fds + i));
        event_set(&ev_dns, *(write_fds + i), EV_WRITE | EV_PERSIST, ev_callback,
                (void *) (channel));
    }
